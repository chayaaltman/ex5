...
public class FINALVARAIBLE {
    ...
    private static final String VARIABLE_BODY_REGEX= valNameRegex+"(?: *= *(\\S.*))?(?:, *("+valNameRegex+")(?: *= *(\\S.*))?)* *;$";
    ...
    private void checkBody(String type, String body, varProperties scope, Boolean is_final) throws Exception {
        ...
        while (matcher.find()) {
            String variableName = matcher.group(1).trim(); // Variable name
            ...
            if (value!=null){
                if (value.matches(valNameRegex)){
                    ...
                    isVarAssigned(scope, value, parserFile.type.valueOf(type));
                }
            }
    ...
    public boolean isVarAssigned(varProperties scope, String var, type my_type){
        if (scope==varProperties.GLOBAL){
            for (type type : globalValMap.keySet()) {
                if (my_type==type) {
